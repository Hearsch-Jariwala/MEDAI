...
def scaling(data, data_opt):
    ...
    col_options = col1.selectbox(
        "Options", ["Select Columns", "Select All Except"], key="col_options"
    )
    ...
    col3.markdown("#")
    add_pipeline = col3.checkbox(
        "Add To Pipeline", True, key="scaling_add_pipeline"
    )
    ...
    columns = st.multiselect(col_options, variables, default_dict[default])
